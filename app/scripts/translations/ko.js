...
NODE_Warning                : 'Aura Wallet을 통해 연결하기 위해서는 HTTPS 노드가 필요합니다. 모든 노드에 연결하려면 [MyEtherWallet 저장소를 다운로드하고 로컬로 실행] (https://github.com/kvhnuke/etherwallet/releases/latest) 할 수 있습니다. 추가로 무료 SSL 인증서를 [LetsEncrypt](https://letsencrypt.org/)에서 받으실 수 있습니다. ',
NODE_Name                   : '노드 명 ',
NODE_Port                   : '노드 포트 ',
...
/* View Wallet Details */
VIEWWALLET_Subtitle         : '다른 버전의 개인 키를 다운로드하거나 종이 지갑 정보를 다시 인쇄할 수 있습니다. [계좌를 Geth/Mist로 가져오기](http://ethereum.stackexchange.com/questions/465/how-to-import-a-plain-private-key-into-geth/)를 진행할 때 필요합니다. 잔액을 확인하려면 [explore.auraledger.com](https://explore.auraledger.com/)과 같은 블록체인 조회 서비스를 사용하는 것을 권장합니다. ',
...
HELP_Remind_Desc_1          : '**이더리움, Aura Wallet & MyEtherWallet CX, 그리고 우리가 사용하는 JavaScript 라이브러리는 활발하게 개발되고 있습니다. ** 우리가 철저하게 테스트를 진행하여 수만 개의 지갑이 전 세계 사람들에 의해 성공적으로 만들어졌지만 예상치 못한 일이 발생하여 ETH를 잃을 가능성이 항상 존재합니다. 항상 조심해주세요. 만약 무슨 일이 발생한다면, 유감스럽지만 **우리는 잃어버린 이더리움에 대한 책임을 지지 않습니다.** ',
HELP_Remind_Desc_2          : 'Aura Wallet 과 MyEtherWallet CX는 웹 지갑이 아닙니다. 계정을 만들거나 우리에게 당신이 가진 이더리움을 주면 안 됩니다. 모든 데이터는 당신의 컴퓨터와 브라우저에 남아 있을 것입니다. 우리는 당신이 쉽게 당신의 정보를 만들고, 저장하고, 접근하고 블록체인과 상호 작용할 수 있도록 도울 것입니다. ',
...
HELP_3_Desc_3               : 'Aura Wallet 에서 "지갑 정보 보기" 페이지로 이동합니다. ',
...
HELP_8_Desc_2               : '만약 Aura Wallet 이 다운된다면, 다른 대안 도구 (geth 또는 Ethereum Wallet / Mist 등)을 사용해야 합니다. MyEtherWallet 에서 이더리움을 꺼내야 할 필요는 없습니다. 이더리움들은 Aura Wallet 을 통해서 생성된 지갑 내에 존재합니다. ',
...
HELP_16_Desc_1              : '[explore.auraledger.com] (https://explore.auraledger.com/) 와 같은 블록체인 탐색기를 사용할 수 있습니다. 주소를 검색 창에 붙여넣으면 주소와 거래 내역이 표시됩니다. 예를 들어, 우리의 [기부 계정]은 (https://explore.auraledger.com/#/address/0x7cb57b5a97eabe94205c07890be4c1ad31e486a8) 과 같습니다. ',

HELP_17_Title               : '17) 내 지갑에서 잔액이 표시되지 않는 이유는 뭔가요? ',
HELP_17_Desc_1              : '그 이유는 방화벽 때문입니다. 우리가 잔액 정보를 얻고 잔액 정보를 변환하는 데 사용하는 API는 어떤 이유로 든 방화벽에 의해 차단되는 경우가 많습니다. 사용자는 계속해서 트랜잭션을 보낼 수 있습니다. 당신은 단지 잔액을 확인하기 위해 explore.auraledger.com 블록체인 탐색 사이트와 같은 다른 방법을 사용해야 될 수 있습니다. ',
...
HELP_SecCX_Desc_6           : '닉네임과 지갑 주소를 암호화하면 계좌 잔액과 닉네임들을 확인할 때마다 비밀번호를 입력해야 합니다. 보안이 걱정된다면 Chrome 확장 프로그램 대신 Aura Wallet 을 사용해주세요. ',
